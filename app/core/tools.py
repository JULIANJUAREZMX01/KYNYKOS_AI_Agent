...
class ToolExecutor:
    ...
    async def _execute_shell(self, args: Dict[str, Any], ctx: AgentContext) -> str:
        ...
        try:
            ...
            # command is user-controlled, but if OVERDRIVE is on, it's allowed.
            # We still use shell=True here as it's the intended purpose of execute_shell,
            # but we've added a warning about overdrive.
    ...
    async def _git_operation(self, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Parse operation and use cwd instead of shell concatenation
    ...
    async def _search_code(self, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Pass arguments via $args to avoid injection in the command string
            cmd = [
                "powershell", "-NoProfile", "-Command",
                "& { Get-ChildItem -Path $args[0] -Recurse | Select-String -Pattern $args[1] | Select-Object -First 20 }",
                path, query
            ]
    ...
    async def _get_system_stats(self) -> str:
        ...
        try:
            ...
            cpu_load = await asyncio.to_thread(subprocess.check_output, cpu_cmd, text=True)
            ...
            ram_stats = await asyncio.to_thread(subprocess.check_output, ram_cmd, text=True)
            ...
            disk_stats = await asyncio.to_thread(subprocess.check_output, disk_cmd, text=True)
    ...
    async def _network_scan(self, args: Dict[str, Any]) -> str:
        ...
        try:
            # Use platform-specific flag for ping count
    ...
    async def _manage_processes(self, args: Dict[str, Any]) -> str:
        ...
        try:
            if action == "list":
                ...
                cmd = [
                    "powershell", "-NoProfile", "-Command",
                    "Get-Process | Sort-Object CPU -Descending | Select-Object -First 15 Name, Id, CPU, WorkingSet | Format-Table -HideTableHeaders"
                ]
                result = await asyncio.to_thread(subprocess.run, cmd, capture_output=True, text=True)
    ...
    async def _self_repair(self, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if target in ["all", "code"]:
                ...
                result = await asyncio.to_thread(subprocess.run, ["git", "pull", "origin", "main"], capture_output=True, text=True)
                reports.append(f"Git: {result.stdout.strip() or 'Al día'}")
            
            if target in ["all", "deps"]:
                # Check/install dependencies
                logger.info("🔧 KYNIKOS: Verificando dependencias...")
                result = await asyncio.to_thread(subprocess.run, ["pip", "install", "-r", "requirements.txt"], capture_output=True, text=True)
    ...
    async def _system_speak(self, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            asyncio.create_task(asyncio.to_thread(subprocess.run, cmd))
